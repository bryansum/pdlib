...
static void dac_dsp(t_dac *x __attribute__((unused)), t_signal **sp)
{
    t_signal **sp2;
    int channel;
...
static void dac_free(t_dac *x __attribute__((unused))) {}

static void dac_setup(void)
{
    dac_class = class_new(gensym("dac~"), (t_newmethod)dac_new,
        (t_method)dac_free, sizeof(t_dac), CLASS_DEFAULT, 0);
