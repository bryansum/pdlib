...
static void *sigsend_new(t_symbol *s)
{
    ...
    x->x_sym = s;
    x->x_n = sys_schedblocksize;
    x->x_vec = (t_sample *)getbytes(sys_schedblocksize * sizeof(t_sample));
    memset((char *)(x->x_vec), 0, sys_schedblocksize * sizeof(t_sample));
    x->x_f = 0;
    return (x);
}
...
static void *sigreceive_new(t_symbol *s)
{
    ...
    x->x_n = sys_schedblocksize;             /* LATER find our vector size correctly */
    x->x_sym = s;
    x->x_wherefrom = 0;
